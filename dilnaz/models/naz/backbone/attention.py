...
class SemanticGlobalAttention(nn.Module):
    ...
    def forward(
        ...
        has_padding_mask = attention_mask is not None and attention_mask.shape[1] == key_length
        maskless_decode_step = use_cache and query_length == 1 and not has_padding_mask
        maskless_full_causal = (
            not use_cache
            and not has_padding_mask
            and key_length == query_length
            and cache_position == 0
        )
        attention_mask_4d = None
        if not maskless_decode_step and not maskless_full_causal:
        ...
        output = F.scaled_dot_product_attention(
            ...
            is_causal=maskless_full_causal,
            scale=self.scale,
